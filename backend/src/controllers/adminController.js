  ...
  try {
    const [users, tasks, withdrawals, earnings] = await Promise.all([
      ...
      pool.query(
        `SELECT COALESCE(SUM(reward_earned), 0) AS total
         FROM task_completions WHERE status = 'approved'`
      ),
    ]);
    ...
    res.json({
      ...
      stats: {
        ...
        active_tasks:             Number(tasks.rows[0].count),
        pending_withdrawals:      Number(withdrawals.rows[0].count),
        pending_withdrawal_total: Number(withdrawals.rows[0].total),
        total_approved_earnings:  Number(earnings.rows[0].total),
      },
    });
  } catch (err) {
    next(err);
...
export async function listUsers(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, username, email, plan, balance, is_verified,
    ...
    res.json({
      success: true,
      data:    rows,
      meta: { page, limit, total: Number(countResult.rows[0].count) },
    });
  } catch (err) {
    next(err);
  }
...
export async function updateUser(req, res, next) {
  try {
    ...
    if (is_banned !== undefined) {
      ...
      setClauses.push(`is_banned = $${values.length}`);
    }
    if (is_verified !== undefined) {
      values.push(is_verified);
      setClauses.push(`is_verified = $${values.length}`);
    }
    ...
    const { rows } = await pool.query(
      `UPDATE users SET ${setClauses.join(', ')} WHERE id = $${values.length}
       RETURNING id, username, email, is_banned, is_verified`,
      values
    );
...
export async function createTask(req, res, next) {
  try {
    const { title, type, reward_amount, min_plan = 'free' } = req.body;

    const { rows } = await pool.query(
      `INSERT INTO tasks (title, type, reward_amount, min_plan)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
...
export async function updateTask(req, res, next) {
  try {
    const { id } = req.params;
    const { title, reward_amount, is_active, min_plan } = req.body;
...
export async function listPendingWithdrawals(req, res, next) {
  try {
...
export async function processWithdrawal(req, res, next) {
  ...
  try {
    ...
    if (withdrawal.status !== 'pending') {
      throw new ValidationError('Withdrawal is not in pending state');
    }
